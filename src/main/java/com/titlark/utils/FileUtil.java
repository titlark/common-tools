...
public class FileUtil {
    ...
    public static boolean write(InputStream inputStream, String outFilePath, int bufferSize) {
        try (BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(outFilePath))) {
            int len = 0;
    ...
    public static boolean copyFile(String inFilePath, String outFilePath, int bufferSize) {
        try (FileInputStream fis = new FileInputStream(inFilePath); FileOutputStream fos = new FileOutputStream(outFilePath)) {
            int len = 0;
            byte[] buffer = new byte[bufferSize];
    ...
    public static boolean split(String filePath, String targetFilePath, int targetFileSize) {
        try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(filePath))) {
            ...
            for (int i = 0; i < times; i++) {
                ...
                bis.read(buffer);
                BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(targetFilePath + "/" + getPrefixName(filename) + i + getSuffixName(filename)));
                bos.write(buffer);
                bos.flush();
                bos.close();
            }
    ...
    public static boolean merge(List<String> filePathList, String targetFilePath) {
        try (BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(targetFilePath))) {
